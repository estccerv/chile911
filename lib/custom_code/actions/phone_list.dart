...
Future<List<CountryStruct>> phoneList(String? locale) async {
  ...
  sealedCountries.forEach((country) {
    var countryCodeInLowerCase = country.codeShort.toLowerCase();
    countries.add(CountryStruct(
        name: country.translation(basicLocale).name,
        commonName: country.namesCommonNative(),
        code: country.codeShort,
        dialCode: country.idd.phoneCode(),
        emoji: country.emoji,
        search: [
          country.translation(basicLocale).name,
          country.codeShort,
